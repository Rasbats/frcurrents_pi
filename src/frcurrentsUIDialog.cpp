...
static wxString TToString(const wxDateTime date_time, const int time_zone)
{
    ...
    switch (time_zone) {
    case 0:
        return t.Format(" %a %d-%b-%Y  %H:%M ", wxDateTime::Local)
            + "LOC"; //:%S
    case 1:
    default:
...
void frcurrentsUIDialog::OnInformation(wxCommandEvent& event)
{
    ...
    if (!m_bFound)
        wxMessageBox("No Information Found", "Internet Browser");
}
...
void frcurrentsUIDialog::OnSelectData(wxCommandEvent& event)
{
    ...
    wxDirDialog* d = new wxDirDialog(
        this, _("Choose a directory"), "", 0, wxDefaultPosition);
...
void frcurrentsUIDialog::LoadTCMFile()
{
    ...
    if (TCDir == wxEmptyString) {
        ...
        wxDirDialog* d = new wxDirDialog(
            this, _("Choose the tcdata directory"), "", 0, wxDefaultPosition);
        if (d->ShowModal() == wxID_OK) {
            m_dirPicker1->SetValue(d->GetPath());
            TCDir = m_dirPicker1->GetValue();
        }
#else
        wxString tc = "/storage/emulated/0/Android/data/org.opencpn.opencpn/"
                      "files/tcdata";
        m_dirPicker1->SetValue(tc);
        TCDir = tc;
        ...
        if (TIDE_PLOT == m_plot_type) {
            if (!((tcv[i] > val) == wt) && (i > 0)) // if tide flow sense change
            {
                ...
                ptcmgr->GetHightOrLowTide(tt, BACKWARD_TEN_MINUTES_STEP,
                    BACKWARD_ONE_MINUTES_STEP, tcv[i], wt, pIDX->IDX_rec_num,
                    tcvalue, tctime);
                if (tctime
                    > tt_localtz) { // Only show events visible in graphic
                    // presently shown
...
double frcurrentsUIDialog::CalcCurrent(
    double VE, double ME, double spRate, double npRate, double coefficient)
{
    // y = mx + c
...
bool frcurrentsUIDialog::LoadStandardPorts()
{
    ...
    wxString wxPORT_NUMBER, wxPORT_NAME, wxPORT_LAT, wxPORT_LON, PMVE, PMME,
        BMVE, BMME, IDX;
        ...
        for (TiXmlElement* f = e->FirstChildElement(); f;
            ...
            if (!strcmp(f->Value(), "PLUS_6")) {
                ...
                my_positions.push_back(
                    my_position); // End of diamond information. Push to diamond
                                  // object
            }
