...
void frcurrentsUIDialog::SetScaledBitmaps(double scalefactor) {
  ...
  int w, h;
  w = 32 * scalefactor; // 32x32 is the standard size of bitmaps
  h = 32 * scalefactor;
#ifdef ocpnUSE_SVG
...
//  end new date
  ...
  GetCurrentsData(s);
    ...
    if (m_bUseBM)
      m_staticText211->SetLabel(m_dt.Format("%H:%M") + "    " + label_lw[button_id]);
    else
      m_staticText211->SetLabel(m_dt.Format("%H:%M") + "    " + label_array[button_id]);
...
void frcurrentsUIDialog::CalcHW(int PortCode) {
  ...
  if (h1 == 0)
    m_stz.Printf("Time Zone : UTC %+03d", h);
  else
    m_stz.Printf("Time Zone : UTC %+03d:%02d", h, h1);
...
void frcurrentsUIDialog::OnChooseTideButton(wxCommandEvent& event) {
  ...
  if (s == "LE HAVRE, France" || s == "LA ROCHELLE - LA PALLICE, France") {
    m_staticText211->SetLabel(s1 + "    " + label_lw[button_id]);
  } else {
    m_staticText211->SetLabel(s1 + "    " + label_array[button_id]);
  }
  m_bChooseTide = true;
  m_bPrev = false;
  m_bNext = false;
