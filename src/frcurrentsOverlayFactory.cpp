...
void frcurrentsOverlayFactory::DrawGL(PlugIn_ViewPort &piVP) {
  ...
  wxColour frcurrentsOverlayFactory::GetSpeedColour(double my_speed) {
    ...
    wxColour c_yellow_orange =
        wxColour(m_dlg.myUseColour[2]);                  // 253, 184, 19);
  ...
  bool frcurrentsOverlayFactory::drawCurrentArrow(
      int x, int y, double rot_angle, double scale, double rate) {
    ...
    for (int i = 1; i < ctcmgr->Get_max_IDX() + 1; i++) {
      ...
      if (((type == 'c') || (type == 'C')) && (1 /*pIDX->IDX_Useable*/)) {
        ...
        if (!b_dup && myRect.Contains(cpoint.x, cpoint.y)) {
          if (ctcmgr->GetTideOrCurrent15(myTimeNow, i, tcvalue, dir,
                                         bnew_val)) {
            if (type == 'c' || type == 'C') {
              ...
              if (m_bShowDirection) {
                snprintf(sbuf, 19, "%03.0f", dir);
                m_dc->DrawText(wxString(sbuf, wxConvUTF8), pixxc,
                               pixyc + shift);
              }
            }
          }
